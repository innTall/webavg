...
def get_trades(trades):
  ...
  frame_trades = df(agg_trade_list)
  new_date = []
  for time in frame_trades['T']:
    readable = datetime.fromtimestamp(int(time/1000))
    new_date.append(readable)
  frame_trades.columns = [
    'date', 'price', 'active', 'a4', 'a5', 'a6', 'maker', 'a8']
  frame_trades['date'] = df(new_date)
  frame_trades = frame_trades.drop(['a4', 'a5', 'a6', 'a8'], axis=1)
  frame_trades['price'] = pd.to_numeric(frame_trades['price'])
  frame_trades['active'] = pd.to_numeric(frame_trades['active'])
 
  return frame_trades
  frame_trades = get_trades(trades)
# export_csv = frame_trades.to_csv(
#     r'C:\Users\Usuario\downloads\frame_trades.csv', index = True, header=True)
# print(frame_trades.info())

def base_table_trades(frame_trades):
  frame2t = df(frame_trades, columns=[
      'date', 'price', 'active', 'maker', 'market', 'buy', 'sell'])
  frame2t['maker'].replace([1, 0], [1, -1], inplace=True)
  frame2t['market'] = round(frame2t['price'] * (
      frame2t['active'] * frame2t['maker']), 4)
  frame2t['buy'] = np.array(frame2t['market'])
  frame2t['sell'] = np.array(frame2t['market'])
  
  return frame2t
  frame2t = base_table_trades(frame_trades)
# export_csv = frame2t.to_csv(
#     r'C:\Users\Usuario\downloads\frame2t.csv', index = True, header=True)
# print(frame2t.info())

def table_buy_sell(frame2t):
  frame3t = frame2t[frame2t['sell'] > 0] # выбор значений - бай
  frame3t = df(frame3t, columns=[
      'date', 'price', 'market', 'active', 'sell', 'order']) # тип ордера
  ...
  frame4t = df(frame4t, columns=['price', 'market', 'active', 'buy'])
  frame4t = np.abs(frame4t) # убрать знак "-"
  frame4t['date'] = frame2t['date'] # выравнивание фреймов для даты
  frame5t = df(frame4t, columns=[
      'date', 'price', 'market', 'active', 'buy', 'order']) # селл-таблица
  ...
  frame7t = df(frame6t, columns=[
      'date', 'price', 'market', 'active', 'buy', 'sell', 'order'])

  return frame7t
  frame7t = table_buy_sell(frame2t)
# export_csv = frame7t.to_csv(
#     r'C:\Users\Usuario\downloads\frame7t.csv', index = True, header=True)
# print(frame7t.info())

def price_trade_params(frame7t):
  last_price = round(frame7t['price'].iloc[-1], 8)
  min_price = round(min(frame7t['price']), 8)
  max_price = round(max(frame7t['price']), 8)
  aver_price = round(sum(
      (np.abs(frame7t['market'])) / sum(frame7t['active'])), 8)
  ...
  return (last_price, min_price, max_price, aver_price, buy_down, sell_down,
        buy_up, sell_up)
  price_trade = price_trade_params(frame7t)
  # export_csv = frame7t.to_csv(
  #     r'C:\Users\Usuario\downloads\frame7t.csv', index = True, header=True)
  #print(price_trade)

def scale_perc_trade(price_trade, y_ticks=15):
  (last_price, min_price, max_price, aver_price, buy_down, sell_down,
        buy_up, sell_up) = price_trade
  ...
  scale_percent = np.hstack((up_scale, down_scale))

  return scale_percent
  scale_percent = scale_perc_trade(price_trade, y_ticks=15)
  #print(scale_percent)

def time_trade_params(frame7t, period=15):
...
def volume_trades_params(frame7t, sum_15m):
  ...
  aver_sell15_order = round(sum(frame7t['sell']) / sum_15m, 4)
  max_sell_order = round(max(frame7t['sell']), 4) # макс размер селл-ордера
  max_buysell_order = max(max_buy_order, max_sell_order) # абсолютный макс
  min_aver_buysell = min(aver_buy15_order, aver_sell15_order) # средний мин
  aver_volume = (sum_buy_order + sum_sell_order) / 2 # среднее ордеров по
  aver_15m_vol = round((aver_buy15_order + aver_sell15_order) / 2, 4)

  return (aver_buy15_order, max_buy_order, aver_sell15_order, max_sell_order,
        max_buysell_order, min_aver_buysell, aver_volume, aver_15m_vol)
  volume_trades = volume_trades_params(frame7t, sum_15m)
  #print(volume_trades)

def tables_max_trades(frame7t, volume_trades):
  aver_15m_vol = volume_trades[7]
  frame8t = df(frame7t, columns=[
      'date', 'price', 'market', 'buy', 'sell', 'order'])
  frame9t = frame8t[frame8t['market'] > aver_15m_vol]
  frame10t = df(frame9t, columns=[
      'date', 'price', 'market', 'buy', 'sell', 'order'])
  
  return frame10t
  frame10t = tables_max_trades(frame7t, volume_trades)
  
  # export_csv = frame10t.to_csv (
  # r'C:\Users\Usuario\downloads\frame10t.csv', index = True, header=True)
  
#   return (total, frame8t, frame10t, total2, last_price, min_price, max_price,
#  aver_price, scale_percent,
#   buy_down, sell_down, buy_up, sell_up, first_time, last_time,
#  aver_buy15_order, max_buy_order,
#   aver_sell15_order, max_sell_order, max_buysell_order, min_aver_buysell,
#  aver_volume, aver_15m_vol)

#   (pivtab, f8t, f10t, pivtab2, lastpt, minpr, maxpr, averpr, percscat, buyd,
#  selld, buyu,
...
#                'close', 'active', 'market', 'buy', 'sell', 'diff', 'perc',
#  'ao', 'rsi'])
...
#   total = frame2c.pivot_table(['buy', 'sell', 'market', 'diff', 'perc'],
#  ['price'], aggfunc='sum')
...
#   if buy_ticks > (y_ticks - 2): buy_ticks = (y_ticks - 2)
#   sell_ticks = y_ticks - buy_ticks
#   one_tick_buy = round(((last_price - min_price) / last_price) * 100 /
#  (buy_ticks - 1), 2)
#   one_tick_sell = round(((max_price - last_price) / last_price) * 100 /
#  (sell_ticks - 1), 2)
#   res_buy = list(map(lambda var: var * one_tick_buy / 100, range(buy_ticks)))
#  # расчет вниз
#   res_sell = list(map(lambda var: var * one_tick_sell / 100, range(sell_ticks))
# ) # расчет вверх
#   up_scale = last_price + last_price * np.array(res_sell) # процентная шкала
#  вверх от текущей цены
#   down_scale = last_price - last_price * np.array(res_buy) # процентная шкала
#  вниз от текущей цены
#   scale_percent = np.hstack((up_scale, down_scale)) # создание массива perc
#  значений
...
#   max_buysell_vol = max(max_buy_vol, max_sell_vol) # макс значение объема
#  (бай/селл)
#   aver_volume = round(sum(frame2c['market']/2/amount), 4) # среднее значение
#  бай/селл объемов за период
...
#   return (frame2c, total, last_price, min_price, max_price, aver_price, 
# scale_percent,
#   buy_down, sell_down, buy_up, sell_up, amount, first_time, last_time,
#  max_volume,
#   max_buy_vol, max_sell_vol, max_buysell_vol, aver_volume, aver_percent,
#  perc_max_buy,
#   perc_max_sell, perc_buysell_max)

#   (f2c, pivtab1, lastpc, mic, mac, avc, perscalec, buyd, selld, buyu, sellu,
#  nums, ft, lt,
#   volmax, volmab, volmas, volbs, volav, averperc, percmab, percmas, percmabs)
#  = get_candles(candles, y_ticks=15)
#   #export_csv = f5c.to_csv (r"C:\Users\Usuario\downloads\f5c.csv", index =
#  True, header=True)
...
#   frame2t = df(frame1t, columns=['date', 'price', 'active', 'maker', 'market',
#  'buy', 'sell'])
#   frame2t['maker'].replace([1, 0], [1, -1], inplace=True)
#   frame2t['market'] = round(frame2t['price'] * (frame2t['active'] * frame2t
# ['maker']), 4)
#   frame2t['buy'] = np.array(frame2t['market'])
#   frame2t['sell'] = np.array(frame2t['market'])
  
#   frame3t = frame2t[frame2t['sell'] > 0] # выбор значений - бай
#   frame3t = df(frame3t, columns=['date', 'price', 'market', 'active', 'sell',
#  'order']) # колонка с типом ордера
#   frame3t['order'] = np.dtype(frame3t['order'], str()) # формат - текст
#   frame3t['order'] = 'sell' # вставить тип ордера
...
#   frame4t = np.abs(frame4t) # убрать знак "-"
#   frame4t['date'] = frame2t['date'] # выравнивание фреймов для даты
#   frame5t = df(frame4t, columns=['date', 'price', 'market', 'active', 'buy',
#  'order']) # селл-таблица
#   frame5t['order'] = np.dtype(frame5t['order'], str()) # формат - текст
#   frame5t['order'] = 'buy' # вставить тип ордера

#   frame6t = frame3t.combine_first(frame5t) # объединить бай и селл таблицы
#   frame7t = df(frame6t, columns=['date', 'price', 'market', 'active', 'buy',
#  'sell', 'order']) # объединенная таблица
#   total = frame7t.pivot_table(['price', 'buy', 'sell'], ['date'], aggfunc='sum')
#   # основные параметры цены
#   last_price = round(frame7t['price'].iloc[-1], 8)
#   min_price = round(min(frame7t['price']), 8)
#   max_price = round(max(frame7t['price']), 8)
#   aver_price = round(sum((np.abs(frame7t['market'])) /
#  sum(frame7t['active'])), 8)
#   # расчет процентной шкалы цен
#   buy_ticks = int((last_price - min_price) * y_ticks // (max_price - min_price))
#   if buy_ticks < 2: buy_ticks = 2
#   if buy_ticks > (y_ticks - 2): buy_ticks = (y_ticks - 2)
...
#   down_scale = last_price - last_price * np.array(res_buy)
#   scale_percent = np.hstack((up_scale, down_scale))
#  # создание массива perc значений
#   # уровни сопр/подд от средней цены
#   downzone = round(((aver_price - min_price) / 3), 8) # 3 зоны вниз от средней
#   buy_down = min_price + downzone # нижняя бай-зона
#   sell_down = aver_price - downzone # нижняя селл-зона
...
#   frame7t['buy'] = frame7t['buy'].fillna(0)
#   sum_buy_order = round(sum(frame7t['buy']), 4) # сумма бай-ордеров за период
#   aver_buy15_order = round(sum(frame7t['buy']) / sum_15m, 4) # средний размер
#  бай-ордера за 15 мин
#   max_buy_order = round(max(frame7t['buy']), 4) # макс размер бай-ордера
#   frame7t['sell'] = frame7t['sell'].fillna(0)
#   sum_sell_order = round(sum(frame7t['sell']), 4) # сумма селл-ордеров за период
#   aver_sell15_order = round(sum(frame7t['sell']) / sum_15m, 4)
#  # средний размер селл-ордера за 15 мин
#   max_sell_order = round(max(frame7t['sell']), 4) # макс размер селл-ордера
#   max_buysell_order = max(max_buy_order, max_sell_order) # абсолютный макс
#  ордера (бай/селл)
#   min_aver_buysell = min(aver_buy15_order, aver_sell15_order) # средний мин
#  ордер (бай/селл) за 15 мин
#   aver_volume = (sum_buy_order + sum_sell_order) / 2 # среднее ордеров по
#  объемам (бай/селл)
#   aver_15m_vol = round((aver_buy15_order + aver_sell15_order) / 2, 4)
#  # средний по 15мин (бай/селл)
#   # последние таблицы по объемам
#   frame8t = df(frame7t, columns=['date', 'price', 'market', 'buy', 'sell',
#  'order']) # почти финальная таблица
#   frame9t = frame8t[frame8t['market'] > aver_15m_vol] # выбор всех ордеров
#  больше значения 15 мин
#   frame10t = df(frame9t, columns=['date', 'price', 'market', 'buy', 'sell',
#  'order'])
#   frame10t.sort_index(ascending=True)
#   total2 = frame10t.pivot_table(['price', 'buy', 'sell'], ['date'],
# aggfunc='sum')
...
#   #export_csv = f8t.to_csv (r'C:\Users\Usuario\downloads\f8t1.csv', index =
#  True, header=True)
